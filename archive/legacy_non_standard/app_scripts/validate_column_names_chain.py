...
print(f"OK: {len(EXPECTED_EXCEL_HEADERS)} colonnes attendues:")
...
try:
    ...
    print(f"OK: {len(db_columns)} colonnes trouvées:")
...
mapping_match = re.search(
    r"column_mapping\s*=\s*\{([^}]+)\}", import_service_code, re.DOTALL
)
if mapping_match:
    print("OK: column_mapping trouvé dans import_service_complete.py")
    ...
    print("WARN: column_mapping non trouvé")

# Chercher le SQL INSERT
insert_match = re.search(
    r"INSERT INTO payroll\.imported_payroll_master\s*\(([^)]+)\)",
    import_service_code,
    re.DOTALL,
)
if insert_match:
    ...
    print(f"\nOK: SQL INSERT trouvé avec {len(insert_cols)} colonnes:")
    ...
    if db_columns:
        ...
        missing_in_insert = (
            set(db_columns) - set(insert_cols) - {"id", "imported_at"}
        )  # ignorer auto
        extra_in_insert = set(insert_cols) - set(db_columns)

        if missing_in_insert:
            print(f"  WARN: Colonnes BD absentes du INSERT: {missing_in_insert}")
        if extra_in_insert:
        ...
        if not missing_in_insert and not extra_in_insert:
            print("  OK: Cohérence parfaite entre INSERT et BD")
else:
    print("WARN: SQL INSERT non trouvé")
...
if col_map_matches:
    print(f"OK: col_map trouvé avec {len(set(col_map_matches))} clés:")
    for key in sorted(set(col_map_matches))[:10]:
        print(f"    - {key}")
else:
    print("WARN: col_map non trouvé (ou syntaxe différente)")
...
if os.path.exists(employees_html_path):
    ...
    sql_cols = re.findall(
        r"COALESCE\([^,]+,\s*\'[^\']*\'\)\s+AS\s+(\w+)", employees_html
    )
    if sql_cols:
        print(f"OK: Colonnes SQL trouvées dans employees.html:")
        for col in sql_cols[:10]:
            print(f"    - {col}")
    else:
        print("WARN: Aucune colonne SQL AS trouvée")
else:
    print("WARN: employees.html non trouvé")
...
if insert_match and db_columns:
    if extra_in_insert:
        issues_found.append(
            f"CRITIQUE: INSERT utilise des colonnes inexistantes: {extra_in_insert}"
        )
