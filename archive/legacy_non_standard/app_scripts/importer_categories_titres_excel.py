...
# -*- coding: utf-8 -*-
"""
Script pour importer les catégories et titres d'emploi depuis le fichier Excel
...
# Ensure repository root is on sys.path so `import app.providers...` works when
# running the script from the project root. The script previously inserted
# the `app` directory which prevented importing the top-level `app` package.
...
try:
    ...
    print("\n" + "=" * 80)
    print("  IMPORT DES CATÉGORIES ET TITRES D'EMPLOI DEPUIS EXCEL")
    print("=" * 80 + "\n")
    ...
    # Créer un dictionnaire de mapping: (source_file, n_de_ligne) -> (categorie, titre)
    print("\n📊 Préparation des données...")
    mapping = {}
    for idx, row in df.iterrows():
        ligne_num = row.get(
            ligne_col, idx + 2
        )  # +2 car ligne 1 = header, ligne 2 = première donnée
    ...
    def update_db(conn):
        ...
        with conn.cursor() as cur:
            for (file, ligne), (cat, titre) in mapping.items():
                # Mettre à jour categorie_emploi
                if cat:
                    sql_cat = """
    ...
    try:
        ...
        print(f"  OK: {updated_cat} lignes mises à jour pour categorie_emploi")
        print(f"  OK: {updated_title} lignes mises à jour pour titre_emploi")
    except Exception as e:
        if "droit refusé" in str(e) or "permission denied" in str(e).lower():
            print(f"\nWARN: Droits insuffisants pour mettre à jour directement.")
            print(f"  Création d'un script SQL à exécuter avec le rôle postgres...")
...
"""
            ...
            # Générer les UPDATE pour chaque ligne
            batch_size = 100
...
"""
            ...
            print(f"\nOK: Script SQL créé: {script_path}")
            print(f"  Nombre de mises à jour: {len(mapping)} lignes")
