...
# -*- coding: utf-8 -*-
"""
Script pour analyser le fichier Excel et compter catégories/titres d'emploi
"""
...
try:
    ...
    # Chercher les colonnes de catégorie et titre d'emploi
    print("🔍 RECHERCHE DES COLONNES CATÉGORIE ET TITRE D'EMPLOI")
    print("-" * 80)

    # Chercher les colonnes qui contiennent "categorie" ou "catégorie"
    cat_cols = [
        col
        for col in df.columns
        if "categorie" in str(col).lower() or "catégorie" in str(col).lower()
    ]
    ...
    print(f"  Colonnes catégorie trouvées: {cat_cols}")
    print(f"  Colonnes titre trouvées: {title_cols}")
    print()

    # Si pas trouvé exactement, chercher avec des patterns plus larges
    if not cat_cols:
        # Chercher toutes les colonnes qui pourraient contenir catégorie
        ...
        potential_cat = [
            ...
            if any(
                ...
                for word in ["categorie", "catégorie", "category"]
            )
        ]
        print(f"  Colonnes potentielles catégorie: {potential_cat}")
    ...
    if cat_cols:
        cat_col = cat_cols[0]
        print(f"📊 ANALYSE DE LA COLONNE: {cat_col}")
        print("-" * 80)
        ...
        if len(value_counts) > 20:
            print(f"    ... et {len(value_counts) - 20} autres catégories")
        print()

    if title_cols:
        title_col = title_cols[0]
        print(f"📊 ANALYSE DE LA COLONNE: {title_col}")
        print("-" * 80)
    ...
    if "employé" in df.columns or "employe" in df.columns:
        emp_col = "employé" if "employé" in df.columns else "employe"
    ...
    if cat_cols and title_cols:
        print("📝 RÉSUMÉ:")
        print(f"  Colonne catégorie: {cat_cols[0]}")
        print(f"  Colonne titre: {title_cols[0]}")
        print(f"  Catégories distinctes: {df[cat_cols[0]].nunique()}")
